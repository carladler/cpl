...
impl<'a> Program<'a>{
	...
	//	members.  They called by Model.rs
	pub fn add_struct(&mut self, name : String) -> usize{
		let s : Struct = Struct::new(name);
		self.structs.push(s);
		return self.structs.len()-1;
	}

	pub fn add_struct_member(&mut self, name : String, initializer : &Vec<Token>, context : usize){
		self.structs.get_mut(context).unwrap().members.push(StructMember::new(name, initializer));
	}
...
impl InstantiateStatement{
	pub fn new(instantiated_struct : Token, struct_name : Token) -> InstantiateStatement{
		InstantiateStatement {
			//cli : cli,
			instantiated_struct : instantiated_struct,
...
impl<'a> Generator<'_>{
	pub fn new(cli : & 'a CLI<'a>) -> Generator<'a>{
		Generator{
			cli : cli,
			code_gen : CodeGen::new(cli),
		}
	}
