...
impl<'a> Model<'a>{
	pub fn new(cli : & 'a CLI) -> Model<'a>
	{
		 Model{
			cli : cli,
			program : Program::new(cli),
			generator : Generator::new(cli),
	...
	pub fn add_struct(&mut self, struct_name : String) -> usize{
		let context = self.program.add_struct(struct_name.clone());
		if self.cli.is_debug_bit(TRACE_MODEL_ADD) {println!("    Model.add_struct {} context={}",struct_name,context);}
		context
	}

	pub fn add_struct_member(&mut self, member_name : String, initializer : &Vec<Token>, context : usize){
		if self.cli.is_debug_bit(TRACE_MODEL_ADD) {println!("    Model.add_struct_member {} init={:?} context={}",member_name, token_list_text(initializer), context);}
		self.program.add_struct_member(member_name.clone(), initializer, context);
	}
	...
	pub fn add_struct_instantiate(&mut self, instantiated_struct:Token, struct_name : &Token){
		...
		let s = InstantiateStatement::new(instantiated_struct, struct_name.clone());
		let f = self.program.current_function();
	...
	pub fn struct_dump(&self){
		self.program.struct_dump();
	}
}
